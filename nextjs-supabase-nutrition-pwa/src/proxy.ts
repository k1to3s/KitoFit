...
export function proxy(req:NextRequest){
 ...
 const supabase=process.env.NEXT_PUBLIC_SUPABASE_URL;let connect="'self' https://cdn.jsdelivr.net https://fastly.jsdelivr.net";let images="'self' data: blob: https://images.pexels.com";
 ...
 const origin=req.headers.get('origin');const normalizeOrigin=(value?:string|null)=>{if(!value)return null;try{return new URL(value).origin;}catch{return value.replace(/\/$/,'');}};const allowed=[normalizeOrigin(process.env.NEXT_PUBLIC_APP_URL),normalizeOrigin(process.env.E2B_SANDBOX_ID?`https://3000-${process.env.E2B_SANDBOX_ID}.e2b.app`:null),normalizeOrigin(process.env.VERCEL_URL?`https://${process.env.VERCEL_URL}`:null),...(process.env.VERCEL?[]:[req.nextUrl.origin,'http://localhost:3000'])].filter(Boolean);
