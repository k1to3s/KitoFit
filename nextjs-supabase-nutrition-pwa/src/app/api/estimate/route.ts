...
async function boundedForm(req:Request){const max=12*1024*1024+16384;const len=Number(req.headers.get('content-length')||0);if(len>max)throw new ApiError(413,'Please choose a photo under 12 MB.');const reader=req.body?.getReader();if(!reader)throw new ApiError(400,'A photo is required.');let size=0;const chunks:Uint8Array[]=[];while(true){const {done,value}=await reader.read();if(done)break;size+=value.length;if(size>max){await reader.cancel();throw new ApiError(413,'Please choose a photo under 12 MB.');}chunks.push(value);}const buffer=Buffer.concat(chunks);return new Response(buffer,{headers:{'content-type':req.headers.get('content-type')||''}}).formData();}
export async function POST(req:Request){try{
 const user=await identity(req,true);await limit(`ai:${user.id}`,10,86400);await limit('ai:global',60,3600);
 const form=await boundedForm(req);const input=z.object({photo:z.instanceof(File).refine(f=>f.size>0&&f.size<=1210241024),consent:z.literal('true')}).strict().parse(Object.fromEntries(form));
