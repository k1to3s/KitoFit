...
export async function GET(req:Request){try{const user=await identity(req);const q=z.object({q:z.string().trim().min(1).max(100),barcode:z.enum(['true','false']).optional()}).strict().parse(Object.fromEntries(new URL(req.url).searchParams));await limit(`foods:${user.id}`,15);const barcode=q.barcode==='true';if(barcode)z.string().regex(/^\d{4,14}$/).parse(q.q);const term=q.q.toLowerCase();
 const local=barcode?[]:[...library,...drinkLibrary].filter(f=>{
   ...
   if(term==='soda'||term==='pop') return ['coca-cola original','diet coke','sports drink','energy drink'].includes(name);
